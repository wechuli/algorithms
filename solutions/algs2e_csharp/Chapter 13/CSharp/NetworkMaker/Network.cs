    ...
    public class Network
    {
        ...
        public void SaveNetwork(string filename)
        {
            ...
            foreach (NetworkNode node in AllNodes)
            {
                ...
                writer.Write($"{node.Name},{node.Location.X},{node.Location.Y}");
                ...
                foreach (NetworkLink link in node.Links)
                {
                    ...
                    writer.Write($",{otherNode.Index},{link.Cost},{link.Capacity}");
                }
                writer.WriteLine();
            }
        ...
        public void FindAllPairsPaths(out int[,] distance, out int[,] via)
        {
            ...
            for (int i = 0; i < numNodes; i++) distance[i, i] = 0;
            ...
            checked
            {
                for (int via_node = 0; via_node < numNodes; via_node++)
                {
                    for (int from_node = 0; from_node < numNodes; from_node++)
                    {
                        for (int to_node = 0; to_node < numNodes; to_node++)
                        {
                            int new_dist =
