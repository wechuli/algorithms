...
namespace NetworkMaker
{
    public partial class Form1 : Form
    {
        ...
        private Cursor AddLinkCursor, FlowCursor;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            FlowCursor = new Cursor(new MemoryStream(Properties.Resources.FlowCursor));
            ...
            ToggleButtons.Add(flowToolStripButton);
            ToggleButtons.Add(minFlowCutToolStripButton);
            ...
            else if (Mode == Modes.MaximalFlow)
            {
                ...
                else
                {
                    ...
                    if ((Node0 != null) && (Node1 != null))
                    {
                        ...
                        // Display the result.
                        string txt = $"Total flow {Node0}-->{Node1}: {flow}";
                        Console.WriteLine(txt);
                        MessageBox.Show(txt);
            ...
            else if (Mode == Modes.MinFlowCut)
            {
                ...
                else
                {
                    ...
                    if ((Node0 != null) && (Node1 != null))
                    {
                        ...
                        // Display the result.
                        string txt = $"Minimal flow cut {Node0}-->{Node1}: {flow}";
                        Console.WriteLine(txt);
                        MessageBox.Show(txt);
        ...
        private void shiftToolStripButton_Click(object sender, EventArgs e)
        {
            try
            {
                ShiftForm dlg = new ShiftForm();
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    int dx = int.Parse(dlg.xShiftTextBox.Text);
                    int dy = int.Parse(dlg.yShiftTextBox.Text);
                    foreach (NetworkNode node in TheNetwork.AllNodes)
                    {
        ...
        private void topologicalSortToolStripButton_Click(object sender, EventArgs e)
        {
            // Find a full ordering.
            List<NetworkNode> ordering = TheNetwork.TopologicalSort();
            networkPictureBox.Refresh();

            // Display the ordering if we found one.
            if (ordering == null)
            {
                toolStripStatusLabel1.Text = "A full ordering isn't possible.";
            }
            else
            {
                string txt = "Ordering:";
                foreach (NetworkNode node in ordering) txt += " " + node.Name;
                toolStripStatusLabel1.Text = txt;
            }
        }

        private void detectCycleToolStripButton_Click(object sender, EventArgs e)
        {
            if (TheNetwork.ContainsCycle())
            {
                toolStripStatusLabel1.Text = "Contains cycles";
            }
            else
            {
                toolStripStatusLabel1.Text = "No cycles";
            }
        ...
        private void twoColorToolStripButton_Click(object sender, EventArgs e)
        {
            if (!TheNetwork.TwoColor()) MessageBox.Show("The network cannot be two-colored.");
            networkPictureBox.Refresh();
        }

        // N-color the network.
        private void nColorToolStripButton_Click(object sender, EventArgs e)
        {
            int numColors = TheNetwork.NColor();
            MessageBox.Show($"Used {numColors} colors.");
            networkPictureBox.Refresh();
        }

        // Use a hill climbing algorithm to color the network.
        private void hillClimbingColoringToolStripButton_Click(object sender, EventArgs e)
        {
            int numColors = TheNetwork.HillClimbingColor();
            MessageBox.Show($"Used {numColors} colors.");
            networkPictureBox.Refresh();
        }

        private void flowToolStripButton_Click(object sender, EventArgs e)
        {
            SelectTool(flowToolStripButton, Modes.MaximalFlow, FlowCursor);
        }

        private void minFlowCutToolStripButton_Click(object sender, EventArgs e)
        {
            SelectTool(minFlowCutToolStripButton, Modes.MinFlowCut, FlowCursor);
        }
