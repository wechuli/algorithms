...
namespace BoidsGravity
{
    class Boid
    {
        ...
        public void Move(List<Boid> boids, List<Point2d> people,
            Point2d target, double targetMass, double personMass,
            double deltaTime, double attractionWgt,
            double repulsionWgt, double targetWgt, double personWgt)
        {
            int numNeighbors = 0;
            ...
            foreach (Point2d person in people)
            {
                ...
                // Calculate repulsive force.
                Vector2d newRepulsion =
                    new Vector2d(person, this.Position);
                double dist = newRepulsion.Length;
                repulsion += newRepulsion *
                    this.Mass * personMass / (dist * dist * dist);
            }
