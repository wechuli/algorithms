...
namespace BoidsGravity
{
    public partial class Form1 : Form
    {
        ...
        private double AttractionWgt = 0;
        private double RepulsionWgt = 0;
        private double TargetWgt = 0;
        private double TargetMass = 0;
        private double PersonWgt = 0;
        private double PersonMass = 0;
        ...
        private void Start()
        {
            ...
            double boidMass = double.Parse(boidMassTextBox.Text);
            for (int i = 0; i < numBoids; i++)
            {
                ...
                Vector2d velocity = new Vector2d(
                    rand.Next(-5, 6),
                    rand.Next(-5, 6));
                Boids.Add(new Boid(boidMass, location, velocity,
                    maxSpeed, neighborDist));
            }

            // Save the weights.
            AttractionWgt = double.Parse(attractionWgtTextBox.Text);
            RepulsionWgt = double.Parse(repulsionWgtTextBox.Text);
            TargetWgt = double.Parse(targetWgtTextBox.Text);
            TargetMass = double.Parse(targetMassTextBox.Text);
            PersonWgt = double.Parse(personWgtTextBox.Text);
            PersonMass = double.Parse(personMassTextBox.Text);
        ...
        private void moveTimer_Tick(object sender, EventArgs e)
        {
            ...
            {
                boid.Move(Boids, People, target,
                    TargetMass, PersonMass, deltaTime,
                    AttractionWgt, RepulsionWgt, TargetWgt, PersonWgt);
            }
