...
namespace TowerOfHanoi
{
    public partial class Form1 : Form
    {
        ...
        private void solveButton_Click(object sender, EventArgs e)
        {
            ...
            resultTextBox.Text = TowersOfHanoi("A", "B", "C", numDisks);
        }

        // Move the top numDisks disks from peg fromPeg to peg toPeg
        // using otherPeg to hold disks temporarily as needed.
        private string TowersOfHanoi(string fromPeg, string toPeg, string otherPeg, int numDisks)
        {
            string result = "";

            // Recursively move the top n - 1 disks from from_peg to other_peg.
            if (numDisks > 1)
                result += TowersOfHanoi(fromPeg, otherPeg, toPeg, numDisks - 1);
            ...
            if (numDisks > 1)
                result += " " + TowersOfHanoi(otherPeg, toPeg, fromPeg, numDisks - 1);
