...
namespace LcaEulerTour2
{
    public class TreeNode
    {
        ...
        public int EulerTourFirstLocation = -1;
        public int EulerTourLastLocation = -1;
        public int Depth = 0;
        ...
        public TreeNode FindLca(List<TreeNode> tour, TreeNode node1, TreeNode node2)
        {
            ...
            if (node1.EulerTourLastLocation <= node2.EulerTourFirstLocation)
            {
                // Node1 is to the left of node2.
                location1 = node1.EulerTourLastLocation;
                location2 = node2.EulerTourFirstLocation;
            }
            else if (node1.EulerTourFirstLocation >= node2.EulerTourLastLocation)
            {
                // Node1 is to the right of node2.
                location1 = node2.EulerTourLastLocation;
                location2 = node1.EulerTourFirstLocation;
            }
            else if (node1.EulerTourFirstLocation >= node2.EulerTourFirstLocation)
            {
        ...
        {
            if (EulerTourFirstLocation < 0) EulerTourFirstLocation = tour.Count;
            EulerTourLastLocation = tour.Count;
            tour.Add(this);
            if (Children.Count > 0)
            {
                foreach (TreeNode child in Children)
