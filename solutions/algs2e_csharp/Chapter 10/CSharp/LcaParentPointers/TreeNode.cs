...
namespace LcaParentPointers
{
    public class TreeNode
    {
        ...
        public TreeNode FindLca(TreeNode node1, TreeNode node2)
        {
            ...
            for (TreeNode node = node1; node != null; node = node.Parent)
                node.Marked = true;

            // Mark nodes above node2 until we find a marked node.
            TreeNode lca = null;
            for (TreeNode node = node2; node != null; node = node.Parent)
            {
                if (node.Marked)
                {
                    lca = node;
            ...
            // Unmark nodes above node2 until we find an unmarked node.
            for (TreeNode node = node2; node != null; node = node.Parent)
            {
