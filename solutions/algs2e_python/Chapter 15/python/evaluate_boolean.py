...
def evaluate_expression(expression):
    ...
    if expression.upper() == "T":
        return True
    if expression.upper() == "F":
        return False
    raise ValueError(f"Unable to parse literal {expression}")
...
class App:
    ...
    def __init__(self):
        # Make the widgets.
        self.window = tk.Tk()
        self.window.title("evaluate_boolean")
        ...
        self.expression_entry.insert(tk.END, "T&(-F|T)")
