...
class Node:
    ...
    def draw(self, canvas, show_text, fill_color, outline_color, text_color):
        """ Draw the node."""
        # If the node is colored, use its color.
        if self.is_colored:
    ...
    def neighbor_has_color(self, color):
        """ Return True if a neighbor has the indicated color."""
        for link in self.links:
            neighbor = link.node1
            if ((neighbor.is_colored) and (neighbor.back_color == color)):
                return True
        return False
...
class Network:
    ...
    def uncolor_nodes(self):
        """ Mark all nodes as not colored."""
        for node in self.all_nodes:
            node.is_colored = False
    ...
    def make_back_links(self):
        """ Make back links."""
        for node in self.all_nodes:
            for link in node.links:
                link.node1.back_links.append(link)
    ...
    def topological_sort(self):
        ...
        # Build each node's after_me list and set its num_before_me count.
        for node in self.all_nodes:
            node.num_before_me = 0
            node.visited = False
        for node in self.all_nodes:
            for link in node.links:
                link.node1.num_before_me += 1
        ...
        ready = []
        for node in self.all_nodes:
            if node.num_before_me == 0:
                ready.append(node)

        # Repeat until the ready list is empty.
        while len(ready) > 0:
            # Remove the first node from the ready list.
            next_node = ready.pop(0)
            ordering.append(next_node)
            next_node.text = f"{len(ordering)}"
            next_node.visited = True

            # Decrement the num_before_me count for this node's neighbors.
            for link in next_node.links:
                to_node = link.node1
                to_node.num_before_me -= 1

                # If the link now has no predecessors,
                # move it to the ready list.
                if to_node.num_before_me == 0:
                    ready.append(to_node)

        # If any node has a nonzero num_before_me count,
    ...
    def contains_cycle(self):
        ...
        # See if we can perform a topological sort.
    ...
    def two_color(self):
        ...
        first_node.is_colored = True
        first_node.back_color = color1
        colored.append(first_node)
        ...
        while len(colored) > 0:
            ...
            neighbor_color = color1
            if node.back_color == color1:
            ...
            for link in node.links:
                ...
                if neighbor.is_colored:
                    ...
                    if neighbor.back_color == node.back_color:
                        return False
                    ...
                    neighbor.is_colored = True
                    neighbor.back_color = neighbor_color
    ...
    def n_color(self):
        ...
        for num_colors in range(3, 5):
            ...
            if self.assign_color(colors, 0):
                # This worked. Return the number of colors used.
                return num_colors

        raise Exception("Could not find a coloring")

    def assign_color(self, colors, index):
        """ Assign colors for node number index. Return True if a coloring is possible."""
        # If all of the nodes are colored,
        # then see if this coloring works.
        if index == len(self.all_nodes):
            for test_node in self.all_nodes:
                if test_node.neighbor_has_color(test_node.back_color):
                    return False
            return True
        ...
        for color in colors:
            ...
            if not node.neighbor_has_color(color):
                # Try this color.
                node.back_color = color

                # Assign the other nodes' colors.
                # If the assignment works, return True.
                if self.assign_color(colors, index + 1):
                    return True
    ...
    def hill_climbing_color(self):
        ...
        for node in self.all_nodes:
            ...
            for i in range(len(colors)):
                ...
                if not node.neighbor_has_color(colors[i]):
                    node.is_colored = True
                    node.back_color = colors[i]
                    if i > max_color:
                        max_color = i
                    break
            ...
            if not node.is_colored:
                node.is_colored = True
                node.back_color = "white"

        # Return the number of colors used.
        return max_color + 1

    def maximal_flow(self, node0, node1):
        """
        Perform a maximal flow calculation and
        return the total flow between the two nodes.
        """
        # Reset the network.
        self.reset_network()
        self.make_back_links()
        ...
        # Set has_flow = True for all links so they show their flows.
        ...
        flow = 0
        for link in node0.links:
            flow += link.flow
        return flow

    def minimal_flow_cut(self, node0, node1):
        """ Mark links that form a minimal flow cut."""
        # Calculate maximal flows.
        flow = self.maximal_flow(node0, node1)

        # Reset the node and link visited flags.
        self.deselect_nodes()
        self.deselect_links()
        ...
        while len(stack) > 0:
            ...
            for link in node.links:
                ...
                if ((not neighbor.visited) and (link.flow < link.capacity)):
                    ...
                    stack.append(neighbor)
                    neighbor.visited = True
                    neighbor.back_color = "pink"

            # Examine the node's back_links.
            for link in node.back_links:
        ...
        for node in self.all_nodes:
            if not node.visited:
                node.back_color = "lightgreen"
            
        # Mark the links that are part of the minimal flow cut.
        for node in self.all_nodes:
            for link in node.links:
                link.visited = (link.node0.visited and not link.node1.visited)
...
class App:
    ...
    def __init__(self):
        ...
        self.buttons = []
        self.images = []
        toolstrip1 = tk.Frame(self.window)
        toolstrip1.pack(padx=5, pady=(5, 0), side=tk.TOP, fill=tk.X)

        self.deselect_button = self.make_button(toolstrip1, "deselect.png", self.deselect)
        self.make_separator(toolstrip1)
        self.add_node_button = self.make_button(toolstrip1, "add_node.png", self.add_nodes)
        self.add_link_button = self.make_button(toolstrip1, "add_link.png", self.add_links)
        self.add_link2_button = self.make_button(toolstrip1, "add_link2.png", self.add_links2)
        self.make_separator(toolstrip1)
        self.df_traversal_button = self.make_button(toolstrip1, "df_traversal.png", self.df_traversal)
        self.bf_traversal_button = self.make_button(toolstrip1, "bf_traversal.png", self.bf_traversal)
        self.connected_components_button = self.make_button(toolstrip1, "connected_components.png", self.connected_components)
        self.spanning_tree_button = self.make_button(toolstrip1, "spanning_tree.png", self.spanning_tree)
        self.minimal_spanning_tree_button = self.make_button(toolstrip1, "minimal_spanning_tree.png", self.minimal_spanning_tree)
        self.any_path_button = self.make_button(toolstrip1, "any_path.png", self.any_path)
        self.label_setting_tree_button = self.make_button(toolstrip1, "label_setting_tree.png", self.label_setting_tree)
        self.label_setting_path_button = self.make_button(toolstrip1, "label_setting_path.png", self.label_setting_path)
        self.label_correcting_tree_button = self.make_button(toolstrip1, "label_correcting_tree.png", self.label_correcting_tree)
        self.label_correcting_path_button = self.make_button(toolstrip1, "label_correcting_path.png", self.label_correcting_path)
        self.all_pairs_button = self.make_button(toolstrip1, "all_pairs.png", self.all_pairs)

        toolstrip2 = tk.Frame(self.window)
        toolstrip2.pack(padx=5, pady=(5, 0), side=tk.TOP, fill=tk.X)
        self.topo_sort_button = self.make_button(toolstrip2, "topo_sort.png", self.topo_sort)
        self.detect_cycle_button = self.make_button(toolstrip2, "detect_cycle.png", self.detect_cycle)
        self.two_color_button = self.make_button(toolstrip2, "two_color.png", self.two_color)
        self.n_color_button = self.make_button(toolstrip2, "n_color.png", self.n_color)
        self.hill_climbing_color_button = self.make_button(toolstrip2, "hill_climbing_color.png", self.hill_climbing_color)
        self.flows_button = self.make_button(toolstrip2, "flows.png", self.flows)
        self.min_flow_cut_button = self.make_button(toolstrip2, "min_flow_cut.png", self.min_flow_cut)
    ...
    def topo_sort(self):
        ...
        # Find a full ordering.
        ordering = self.the_network.topological_sort()
        self.draw_canvas()

        # Display the ordering if we found one.
        if ordering == None:
            self.status_label["text"] = "A full ordering isn't possible."
        else:
            txt = "Ordering:"
    ...
    def detect_cycle(self):
        ...
        if self.the_network.contains_cycle():
            self.status_label["text"] = "Contains cycles"
        else:
            self.status_label["text"] = "No cycles"

        self.the_network.reset_network()
        self.draw_canvas()

    def two_color(self):
        """ See if the network is two-colorable."""
        if not self.the_network.two_color():
            self.status_label["text"] = "The network cannot be two-colored."
        else:
    ...
    def n_color(self):
        """ N-color the network."""
        num_colors = self.the_network.n_color()
        self.status_label["text"] = f"Used {num_colors} colors."
        self.draw_canvas()

    def hill_climbing_color(self):
        """ Use a hill climbing algorithm to color the network."""
        num_colors = self.the_network.hill_climbing_color()
        self.status_label["text"] = f"Used {num_colors} colors."
        self.draw_canvas()

    def flows(self):
        self.select_tool(self.flows_button, Modes.flows, "Flows")

    def min_flow_cut(self):
        self.select_tool(self.min_flow_cut_button, Modes.min_flow_cut, "Min Flow Cut")
            ...
            if node == None:
                ...
                if ((self.node0 != None) and (self.node1 != None)):
                    ...
                    if self.node0 == self.node1:
                        ...
                        flow = self.the_network.maximal_flow(self.node0, self.node1)

                        # Redraw.
                        self.draw_canvas()

                        # Display the result.
                        txt = f"Total flow {self.node0}-->{self.node1}: {flow}"
                        self.status_label["text"] = txt

                        # We're done with these nodes.
                        self.node0 = None
                        self.node1 = None
        elif self.mode == Modes.min_flow_cut:
            # See if there is a node here.
            node = self.the_network.find_node(location)
            if node == None:
                beep()
                ...
                if ((self.node0 != None) and (self.node1 != None)):
                    ...
                    if self.node0 == self.node1:
                        ...
                        flow = self.the_network.minimal_flow_cut(self.node0, self.node1)

                        # Redraw.
                        self.draw_canvas()

                        # Display the result.
                        txt = f"Minimal flow cut {self.node0}-->{self.node1}: {flow}"
                        self.status_label["text"] = txt
