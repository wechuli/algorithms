...
def recursive_interpolation_search(values, min, max, target):
    """
    ...
    while min <= max:
        ...
        if min <= max:
            index, num_steps = recursive_interpolation_search(values, min, max, target)
...
class App:
    ...
    def __init__(self):
        self.window = tk.Tk()
        self.window.title("recursive_interpolation_search")
    ...
    def find(self):
        ...
        index, steps = recursive_interpolation_search(self.items, 0, len(self.items), target)
