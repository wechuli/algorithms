...
def recursive_linear_search(values, target, index):
    """
    ...
    """
    # Note that this is not a very good approach. The non-recursive
    # version is simpler and doesn'thave problems exceeding the
    # maximum allowed depth of recursion (which is 1,000 by default).
    steps = 1
    ...
    index, num_steps = recursive_linear_search(values, target, index + 1)
    return index, steps  + num_steps
...
class App:
    ...
    def __init__(self):
        self.window = tk.Tk()
        self.window.title("recursive_linear_search")
    ...
    def find(self):
        ...
        index, steps = recursive_linear_search(self.items, target, 0)
