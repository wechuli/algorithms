...
class MyHashTable:
    ...
    def add(self, key, value):
        ...
        stride = self.find_stride(key)

        # Keep track of the number of probes for the current item.
        num_probes_this_item = 0

        num_probes = 0
        while True:
            num_probes += 1
            num_probes_this_item += 1
    ...
    def find_stride(self, seed):
        ...
        rand = random.Random()
        rand.seed(seed)
        return rand.randint(1, self.num_entries)
...
class App:
    ...
    def __init__(self):
        ...
        # Make the widgets.
        self.window = tk.Tk()
        self.window.title("ordered_double_hashing")
