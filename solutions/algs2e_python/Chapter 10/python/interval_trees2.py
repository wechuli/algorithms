...
class IntervalNode:
    ...
    def find_overlapping_intervals2(self, results, xmin, xmax):
        """ Find intervals that overlap the target interval xmin --> xmax."""
        # Check our overlap intervals.
        if xmax <= self.xmid:
            # Use the left overlap list.
            for seg in self.left_overlap:
                if seg.left_point[0] > xmax:
                    break
                results.append(seg)
        elif xmin >= self.xmid:
            # Use the right overlap list.
            for seg in self.right_overlap:
                if seg.right_point[0] < xmin:
                    break
                results.append(seg)
        else:
            # Add all intervals.
        ...
        # Check the children.
        if (xmin < self.xmid) and (self.left_child != None):
            self.left_child.find_overlapping_intervals2(results, xmin, xmax)
        if (xmax > self.xmid) and (self.right_child != None):
            self.right_child.find_overlapping_intervals2(results, xmin, xmax)
...
class App:
    ...
    def __init__(self):
        ...
        # The half width of the target interval.
    ...
    def use_interval_tree(self):
        ...
        hits = []
        self.root.find_overlapping_intervals2(hits, \
            self.test_x - self.target_radius, self.test_x + self.target_radius)
    ...
    def draw(self):
        ...
        # Draw the vertical test area.
        ymax = self.canvas.winfo_height()
        if self.test_x >= 0:
            self.canvas.create_rectangle( \
                self.test_x - self.target_radius, 0, \
                self.test_x + self.target_radius, ymax, fill="lightgreen")
