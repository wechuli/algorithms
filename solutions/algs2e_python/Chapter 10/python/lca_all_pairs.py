...
class TreeNode:
    ...
    def add_to_euler_tour(self, tour):
        ...
        if self.euler_tour_location <0:
            self.euler_tour_location = len(tour)
    ...
    def find_lca(self, tour, node1, node2):
        ...
        location1 = node1.euler_tour_location
        location2 = node2.euler_tour_location
...
class App:
    ...
    def __init__(self):
        ...
        self.window = tk.Tk()
        self.window.title("lca_euler_tour")
        ...
        nodes = [ self.root, node1, node2, node3, node4, node5, node6, \
        ...
        num_nodes = len(nodes)
        self.lcas = [[None for i in range(num_nodes)] for j in range(num_nodes)]
        for i in range(num_nodes):
            for j in range(num_nodes):
                self.lcas[i][j] = self.root.find_lca(self.euler_tour, nodes[i], nodes[j])
                self.lcas[j][i] = self.lcas[i][j]
    ...
    def find_lca(self):
        ...
        if (self.node1 != None) and (self.node2 != None):
            # Find the LCA.
            self.lca_node = self.lcas[self.node1.value][self.node2.value]
